...
import (
	...
	"github.com/kubeedge/kubeedge/edged/pkg/edged/apis"
	edgecadvisor "github.com/kubeedge/kubeedge/edged/pkg/edged/cadvisor"
	"github.com/kubeedge/kubeedge/edged/pkg/edged/clcm"
	edgedconfig "github.com/kubeedge/kubeedge/edged/pkg/edged/config"
	// fakekube "github.com/kubeedge/kubeedge/edged/fake"
	"github.com/kubeedge/kubeedge/edged/pkg/edged/podmanager"
	"github.com/kubeedge/kubeedge/edged/pkg/edged/server"
	"github.com/kubeedge/kubeedge/edged/pkg/edged/status"
	edgedutil "github.com/kubeedge/kubeedge/edged/pkg/edged/util"
	"github.com/kubeedge/kubeedge/edged/pkg/edged/util/record"
	csiplugin "github.com/kubeedge/kubeedge/edged/pkg/edged/volume/csi"
...
func newEdged(enable bool) (*edged, error) {
	...
	//kubeConfig, err := clientcmd.BuildConfigFromFlags(v1alpha1.KubeAPIConfig{}.Master, v1alpha1.KubeAPIConfig{}.KubeConfig)
	//klog.Info("[sxy]:MasterUrl is: %s , kubeconfig is: %s", v1alpha1.KubeAPIConfig{}.Master, v1alpha1.KubeAPIConfig{}.KubeConfig)
...
func (e *edged) syncPod1() {
	time.Sleep(10 * time.Second)

	// 开启四个资源的watch操作
	// pod

	klog.Info("[sxy]start to watch fore resources")
	...
	klog.Info("[sxy]start to receive pod event.")
	...
	klog.Info("[sxy]start to receive configmap event.")
	...
	if err != nil {
		klog.Errorf("failed to list secret: %v", err)
		return
	}
	klog.Info("[sxy]start to receive secret event.")
...
func (e *edged)syncProcess(w watch.Interface, resType string)  {
	switch resType {
	case model.ResourceTypePod:
		klog.Info("[sxy]start to process pod event.")
	...
	case model.ResourceTypeConfigmap:
		klog.Info("[sxy]start to process configmap event.")
		stopCh := make(chan struct{}) //???
		defer close(stopCh)
		for {
			select {
	...
	case model.ResourceTypeSecret:
		klog.Info("[sxy]start to process secret event.")
		stopCh := make(chan struct{}) //???
		defer close(stopCh)
		for {
			select {
